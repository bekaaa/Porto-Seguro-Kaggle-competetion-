...
class xgb_tuner :
	...
	def set_param(self,param,value):
		'''
		call function to set self.__params value, give the param name and value.
		'''
	...
	def predict(self, dtest) :
		'''
		first training the model on the current parameters, then get predictions using cvfolds and finally
		taking the mean of those predictions.
		'''
	...
	def __init_log(self, index) :
		'''
		make sure you have a dir named ./logs/ in current dir.
		setting file name and making an object of the class 'logger' check logger.py for more info
		'''
		assert index >= 0
	...
	def __fpreproc(self, dtrain_, dtest_, param_):
		...
		dtrain, dtest are xgb.DMatrix data. Labels are binary.
		Setting the param 'scale_pos_weight' once.
		'''
